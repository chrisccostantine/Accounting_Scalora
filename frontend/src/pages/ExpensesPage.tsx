...
import type { Expense, ExpenseCategory, Paginated, PaymentMethod } from '../types';
...
const schema = z.object({ title: z.string().min(1), amount: z.coerce.number().nonnegative(), currency: z.string().default('USD'), category: z.enum(categories as [ExpenseCategory, ...ExpenseCategory[]]), paymentMethod: z.enum(methods as [PaymentMethod, ...PaymentMethod[]]), date: z.string(), vendor: z.string().optional(), receiptNumber: z.string().optional(), notes: z.string().optional() });
  ...
  return (
    <>
      ...
      {isLoading ? <Skeleton /> : !data?.items.length ? <EmptyState title="No expense records found." /> : (
        <div className="panel overflow-x-auto"><table className="w-full min-w-[900px]"><thead><tr><th className="table-th">Title</th><th className="table-th">Amount</th><th className="table-th">Category</th><th className="table-th">Date</th><th className="table-th">Vendor</th><th className="table-th"></th></tr></thead><tbody>{data.items.map((item) => <tr key={item.id} className="hover:bg-slate-900/60"><td className="table-td"><button className="font-semibold text-white" onClick={() => { setEditing(item); setOpen(true); }}>{item.title}</button><p className="text-xs text-slate-500">{labelize(item.paymentMethod)}</p></td><td className="table-td">{money(item.amount, item.currency)}</td><td className="table-td">{labelize(item.category)}</td><td className="table-td">{isoDate(item.date)}</td><td className="table-td">{item.vendor || '-'}</td><td className="table-td text-right"><button className="btn-secondary" onClick={() => confirm('Delete this expense?') && remove.mutate(item.id)}><Trash2 size={15} /></button></td></tr>)}</tbody></table></div>
      )}

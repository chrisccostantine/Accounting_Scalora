...
interface Reports {
  summary: { monthlyIncome: number; monthlyExpenses: number; monthlyProfit: number; yearlyProfit: number; averageMonthlyIncome: number; averageMonthlyExpenses: number };
      ...
      {isLoading || !data ? <Skeleton /> : (
        <>
          <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {Object.entries(data.summary).map(([key, value]) => <div className="panel p-5" key={key}><p className="text-sm capitalize text-slate-500">{key.replace(/([A-Z])/g, ' $1')}</p><p className="mt-2 text-2xl font-bold">{money(value)}</p></div>)}
          </section>

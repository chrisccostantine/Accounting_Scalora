      ...
      {isLoading ? <Skeleton /> : !data?.items.length ? <EmptyState title="No clients found." /> : (
        <div className="panel overflow-x-auto">
          <table className="w-full min-w-[980px]">
            <thead><tr><th className="table-th">Name</th><th className="table-th">Services</th><th className="table-th">Fee</th><th className="table-th">Billing</th><th className="table-th">Status</th><th className="table-th">Contact</th><th className="table-th"></th></tr></thead>
            <tbody>{data.items.map((client) => <tr key={client.id} className="hover:bg-slate-900/60"><td className="table-td"><button className="font-semibold text-white" onClick={() => { setEditing(client); setOpen(true); }}>{client.name}</button><p className="text-xs text-slate-500">{client.company}</p></td><td className="table-td">{client.services.map(labelize).join(', ')}</td><td className="table-td">{money(client.monthlyFee, client.currency)}</td><td className="table-td">{labelize(client.billingFrequency ?? 'MONTHLY')}</td><td className="table-td">{labelize(client.status)}</td><td className="table-td">{client.email || client.phone || '-'}</td><td className="table-td text-right"><button className="btn-secondary" onClick={() => confirm('Delete this client?') && remove.mutate(client.id)}><Trash2 size={15} /></button></td></tr>)}</tbody>
          </table>
        </div>
      )}
...
function ClientModal({ client, onClose }: { client: Client | null; onClose: () => void }) {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm<FormValues>({ resolver: zodResolver(schema), defaultValues: client ? { ...client, billingFrequency: client.billingFrequency ?? 'MONTHLY', contractStartDate: isoDate(client.contractStartDate) } : { currency: 'USD', services: ['META_ADS'], status: 'ACTIVE', billingFrequency: 'MONTHLY', contractStartDate: isoDate(), monthlyFee: 0 } });
  return (
    <Modal title={client ? 'Edit Client' : 'New Client'} onClose={onClose}>
      <form className="grid gap-4 md:grid-cols-2" onSubmit={handleSubmit(async (body) => { await save.mutateAsync({ id: client?.id, body }); onClose(); })}>
        ...
        <Field label="Services" error={errors.services?.message}>
          <div className="grid max-h-40 grid-cols-1 gap-2 overflow-y-auto rounded-lg border border-slate-700 bg-slate-900/70 p-3 sm:grid-cols-2">
            {services.map((item) => <label key={item} className="flex cursor-pointer items-center gap-2 text-sm text-slate-200"><input type="checkbox" value={item} {...register('services')} className="h-4 w-4 rounded border-slate-600 bg-slate-800 accent-blue-600" />{labelize(item)}</label>)}
          </div>
        </Field>

...
import type { Client, ClientService, ClientStatus, Paginated } from '../types';
      ...
      {isLoading ? <Skeleton /> : !data?.items.length ? <EmptyState title="No clients found." /> : (
        <div className="panel overflow-x-auto">
          <table className="w-full min-w-[900px]">
            <thead><tr><th className="table-th">Name</th><th className="table-th">Service</th><th className="table-th">Monthly Fee</th><th className="table-th">Status</th><th className="table-th">Contact</th><th className="table-th"></th></tr></thead>
            <tbody>{data.items.map((client) => <tr key={client.id} className="hover:bg-slate-900/60"><td className="table-td"><button className="font-semibold text-white" onClick={() => { setEditing(client); setOpen(true); }}>{client.name}</button><p className="text-xs text-slate-500">{client.company}</p></td><td className="table-td">{labelize(client.service)}</td><td className="table-td">{money(client.monthlyFee, client.currency)}</td><td className="table-td">{labelize(client.status)}</td><td className="table-td">{client.email || client.phone || '-'}</td><td className="table-td text-right"><button className="btn-secondary" onClick={() => confirm('Delete this client?') && remove.mutate(client.id)}><Trash2 size={15} /></button></td></tr>)}</tbody>
          </table>
        </div>
      )}
...
function ClientModal({ client, onClose }: { client: Client | null; onClose: () => void }) {
  const save = useSave<FormValues>('/clients', ['clients']);
  const { register, handleSubmit, formState: { errors } } = useForm<FormValues>({ resolver: zodResolver(schema), defaultValues: client ? { ...client, contractStartDate: isoDate(client.contractStartDate) } : { currency: 'USD', service: 'META_ADS', status: 'ACTIVE', contractStartDate: isoDate(), monthlyFee: 0 } });
  return (
    <Modal title={client ? 'Edit Client' : 'New Client'} onClose={onClose}>
      <form className="grid gap-4 md:grid-cols-2" onSubmit={handleSubmit(async (body) => { await save.mutateAsync({ id: client?.id, body }); onClose(); })}>
        ...
        <Field label="Monthly Fee"><input className="input" type="number" step="0.01" {...register('monthlyFee')} /></Field>

...
export function InvoicesPage() {
  ...
  return (
    <>
      <PageHeader title="Invoices" action={<button className="btn-primary" onClick={() => { setEditing(null); setOpen(true); }}><Plus size={16} /> New Invoice</button>} />
      ...
      {isLoading ? <Skeleton /> : !data?.items.length ? <EmptyState title="No invoices found." /> : (
        <div className="panel overflow-x-auto">
          <table className="w-full min-w-[1040px]">
            ...
            <tbody>{data.items.map((item) => <tr key={item.id} className="hover:bg-slate-900/60"><td className="table-td"><button className="font-semibold text-white" onClick={() => { setEditing(item); setOpen(true); }}>{item.invoiceNumber}</button><p className="text-xs text-slate-500">{item.description}</p></td><td className="table-td">{item.client?.name}</td><td className="table-td">{money(item.amount, item.currency)}</td><td className="table-td">{money(item.paidAmount, item.currency)}</td><td className="table-td">{isoDate(item.dueDate)}</td><td className="table-td"><span className="rounded bg-slate-800 px-2 py-1 text-xs">{labelize(item.computedStatus ?? item.status)}</span></td><td className="table-td"><div className="flex justify-end gap-2"><button className="btn-secondary" onClick={() => setPaying(item)}><CreditCard size={15} /> Pay</button><button className="btn-secondary" onClick={() => confirm('Delete this invoice?') && remove.mutate(item.id)}><Trash2 size={15} /></button></div></td></tr>)}</tbody>
          </table>
        </div>
      )}

...
import type { Client, Income, Paginated, PaymentMethod } from '../types';
import { isoDate, labelize, money } from '../utils/format';

const methods: PaymentMethod[] = ['CASH', 'BANK', 'WHISH', 'OMT', 'TRANSFER', 'OTHER'];
const schema = z.object({ clientId: z.string().min(1), amount: z.coerce.number().nonnegative(), currency: z.string().default('USD'), date: z.string(), paymentMethod: z.enum(methods as [PaymentMethod, ...PaymentMethod[]]), referenceNumber: z.string().optional(), description: z.string().optional(), invoiceNumber: z.string().optional() });
...
export function IncomePage() {
  ...
  const remove = useDelete('/income', ['income']);
  ...
  return (
    <>
      ...
      {isLoading ? <Skeleton /> : !data?.items.length ? <EmptyState title="No income records found." /> : (
        <div className="panel overflow-x-auto"><table className="w-full min-w-[900px]"><thead><tr><th className="table-th">Client</th><th className="table-th">Amount</th><th className="table-th">Date</th><th className="table-th">Method</th><th className="table-th">Reference</th><th className="table-th"></th></tr></thead><tbody>{data.items.map((item) => <tr key={item.id} className="hover:bg-slate-900/60"><td className="table-td"><button className="font-semibold text-white" onClick={() => { setEditing(item); setOpen(true); }}>{item.client?.name}</button><p className="text-xs text-slate-500">{item.description}</p></td><td className="table-td">{money(item.amount, item.currency)}</td><td className="table-td">{isoDate(item.date)}</td><td className="table-td">{labelize(item.paymentMethod)}</td><td className="table-td">{item.referenceNumber || item.invoiceNumber || '-'}</td><td className="table-td text-right"><button className="btn-secondary" onClick={() => confirm('Delete this payment?') && remove.mutate(item.id)}><Trash2 size={15} /></button></td></tr>)}</tbody></table></div>
      )}
...
function IncomeModal({ income, clients, onClose }: { income: Income | null; clients: Client[]; onClose: () => void }) {
  const save = useSave<FormValues>('/income', ['income', 'dashboard']);
  const { register, handleSubmit } = useForm<FormValues>({ resolver: zodResolver(schema), defaultValues: income ? { ...income, date: isoDate(income.date) } : { amount: 0, currency: 'USD', date: isoDate(), paymentMethod: 'BANK', clientId: clients[0]?.id ?? '' } });
